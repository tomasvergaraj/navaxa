...
> **Fase 2 IMPLEMENTADA + DEPLOYADA** (commit `7022d6a`). Editor de campañas: `/marketing` pasó de solo-lectura a gestión. `lib/campaigns.ts` (catálogo de automatizaciones + segmentos/plantillas de broadcast, compartido cliente/servidor). Automatizaciones (recordatorio 24h/1h, reactivación, cumpleaños) con toggle `active`, canal (Email/WhatsApp) y umbral de inactividad editables inline; el job `processInactiveRecalls` ahora lee `conditions.daysSinceLastVisit` (antes 30 fijo), agrupando por tenant. Envío manual (`lib/notifications/broadcast.ts`): segmentos todos / inactivos ≥N días / cumpleañeros del mes, vista previa de conteo (GET), envío en batch respetando degradación de canal y cupo WhatsApp, tope 500 destinatarios, rate-limit 3/h; gate PRO+ (403) con upsell para FREE/STARTER. API `/api/campaigns` (GET list, POST enable-automation), `/[id]` (PATCH active/channel/days, DELETE), `/broadcast` (GET count, POST send). CRUD verificado end-to-end con sesión real; prod quedó en 0 campañas (el dueño las activa desde la UI).
> **Fase 3 IMPLEMENTADA + DEPLOYADA** (commits `9d3777e` caja/productos, `83b3dd7` giftcards). 3.1/3.2: modelos `Product/StockMovement/Sale/SaleItem` (migración `20260717150000` aplicada en prod), gate STARTER+ (`lib/plan-features.ts`); `/productos` con CRUD, entradas/ajustes de stock e historial, alerta de stock mínimo (banner en dashboard); `/caja` (`lib/sales.ts`) con venta rápida de productos+servicios, carrito, método de pago, vendedor opcional, resumen del día por método y anulación con devolución de stock; ventas descuentan stock en transacción con guard optimista; reportes suman "Ventas caja"; STAFF opera caja, BARBER no. 3.3: modelos `GiftCard/GiftCardRedemption` (migración `20260717170000`), gate PRO+; `/giftcards` emite con código único (alfabeto sin ambiguos), email opcional al destinatario (`giftcard_issued`), canje parcial/total por código con guard anti doble-canje, saldo en circulación, anulación. **Compra pública online con Webpay DIFERIDA**: prod usa Webpay real (no testeable sin cobro) y entrelazar el modelo `Payment` compartido era riesgoso; v1 emite en mostrador, ingreso reconocido al emitir (sin doble conteo). CRUD + guards verificados end-to-end con sesión real (venta descuenta/devuelve stock, canje 20k→8k→0 REDEEMED, stock insuficiente 409); prod limpio (0 productos/ventas/giftcards). Features de planes actualizadas (caja STARTER+, giftcards PRO+).
> Follow-ups Fase 3: compra pública de giftcard con Webpay + canje integrado en el checkout de reserva y como método de pago en la Caja.
> **Giftcard como medio de pago en la Caja: HECHO + DEPLOYADO (2026-07-22, commit `6abf819`).** Migración `20260722140000_giftcard_as_payment` (aditiva): `SalePaymentMethod += GIFTCARD`, `sales.giftCardAmount/giftCardId`, `gift_card_redemptions.saleId`. El saldo cubre el total o lo que alcance y el resto se cobra con el método elegido; el canje corre DENTRO de la tx que crea la venta y descuenta stock (`redeemGiftCardTx`), y anular la venta devuelve el saldo con un consumo negativo en el histórico (`refundGiftCardTx`), reactivando la giftcard si había quedado REDEEMED. Contabilidad: el ingreso se reconoce al EMITIR, así que el total del día y "Ventas caja" en reportes suman `total − giftCardAmount` y lo consumido con giftcard va en un chip aparte. El cliente nunca manda `paymentMethod=GIFTCARD` (400); lo deriva el server. Gate PRO+ aunque la caja arranque en Starter. Verificado en prod con sesión real: cobertura total (método→GIFTCARD, 20k→10k), split 15k con saldo 10k (método CARD, `giftCardAmount` 10k, card→0/REDEEMED), 409 sin saldo, 409 giftcard anulada, 404 código inexistente, 400 al forzar el método, y anular ambas ventas devolvió el saldo a 20.000/ACTIVE con canjes netos en 0. Datos de prueba borrados.
> **Giftcard en el checkout de reserva: HECHO + DEPLOYADO (2026-07-22).** Migración `20260722190000_giftcard_on_booking_deposit` (aditiva): `payments.giftCardAmount/giftCardId`, `gift_card_redemptions.paymentId`. El canje NO va en el wizard de reserva sino en `/pagar/[token]`, donde el `Payment` ya existe: `POST /api/public/pay/[token]/giftcard` con el código. Si el saldo cubre el abono completo, el pago queda PAID y la cita SCHEDULED **sin pasar por la pasarela** (no hay dinero nuevo que cobrar); si cubre parte, se guarda `giftCardAmount` y se **recrea** la transacción de la pasarela por el resto, porque el `token_ws` viejo está atado al monto original — por eso `webpay/return` ahora reconcilia contra `chargeableAmount(payment) = amount − giftCardAmount` y no contra `amount` (si no, todo pago parcial se caía por "monto descalzado"). El saldo se devuelve solo si el abono no se concreta: `releasePaymentSlot()` (nuevo `lib/payment-release.ts`, usado por cancelar / fallar / expirar / el job `expirePendingPayments`) hace el refund dentro del mismo claim que marca el pago no vigente, así no se devuelve dos veces. `lib/payment-release.ts` y `lib/giftcards.ts` quedaron libres de `node:crypto` (Web Crypto para el código) porque `instrumentation.ts` los arrastra al bundle Edge. Una giftcard por abono; gate PRO+ en el endpoint y en la UI. Verificado en prod con abono real de $1.000: cobertura total (PAID + cita SCHEDULED, 20.000→19.000, canje con `paymentId`), parcial con saldo 400 (desglose «Abono $1.000 / Giftcard −$400 / A pagar ahora $600», `token_ws` recreado por 600), cancelar devolvió el saldo (400/ACTIVE, consumo neto solo el del mostrador), 400 código corto, 404 inexistente, 409 sin saldo, 409 anulada, 409 reintento sobre pago pagado, 410 abono expirado (libera la hora), 403 con plan STARTER. Falta el commit real de un pago parcial en Webpay (cobra de verdad). Datos de prueba borrados.
> **Fase 4 IMPLEMENTADA + DEPLOYADA** (commits `236a4ab` + fixes `1aefaf4`). 4.1 Color de marca: columna `Tenant.brandColor` (migración `20260717190000_tenant_branding`), `lib/brand-color.ts` (hex→HSL triple + foreground de mayor contraste WCAG); el storefront y el wizard inyectan `--primary/--primary-foreground/--ring` en un ancestro del árbol público (solo si plan PRO+), gate en `/api/tenant` (403), UI en Configuración→Barbería con color picker + vista previa AA + "usar paleta navaxa". 4.2 Marketplace: `/reservar` índice público de barberías activas (logo, ciudad, rating Google+interno, descripción), búsqueda por nombre + filtro por ciudad (form GET sin JS), paginado 24, JSON-LD ItemList, ISR 5min; opt-out por tenant (`marketplaceVisible`, toggle en Configuración, gratis todos los planes); `/reservar` agregado a la whitelist del middleware y al sitemap. Verificado: color guarda/limpia (fix: `null?.toLowerCase()`→undefined no limpiaba), inyección con contraste correcto, opt-out oculta del índice pero el slug directo sigue 200. Features PRO: +"Sitio de reservas con color de marca".
>
> **PLAN COMPLETO (Fases 1–4).** Follow-ups pendientes: compra pública de giftcard con Webpay (bloqueado: prod cobra de verdad, hay que decidir cómo probarlo); segundo color de marca (accent); testimonios reales en landing. El canje en Caja y en el checkout de reserva quedó cerrado el 2026-07-22.

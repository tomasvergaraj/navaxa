...
/**
 * Pago de la compra de giftcard.
 *
 * Con Webpay: POST nativo del formulario a Transbank con `token_ws` (igual que
 * el checkout del abono). Con el proveedor mock (dev): botón que llama al
 * endpoint de confirmación simulada.
 */
export function GiftOrderCheckout({
  token,
  amountLabel,
  webpay,
}: {
  ...
  async function payMock() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/public/giftcards/orden/${token}`, { method: "POST" });
